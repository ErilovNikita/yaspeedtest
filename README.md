# yaspeedtest
YaSpeedTest — это **асинхронный клиент Python**, который точно измеряет скорость интернета, используя те же общедоступные Endpoint'ы, что и официальный [Yandex Internet Speed ​​Test](https://yandex.ru/internet).
...
- Обособленная работа в режиме CLI
...
|Аргумент|Описание| 
|---|---|
|`--count`, `-c`|Количество последовательных замеров *(По умолчанию 1)*|
|`--json`|Вывод в JSON-формате *(По умолчанию `False`)*|
...
### ▶ Запустить один тест *(Используется по умолчанию)*
```bash
yaspeedtest
```
Вывод:
```bash
Ping: 1.22 ms | Download: 1935.44 Mbps | Upload: 1512.91 Mbps
```

### ▶ Запустить несколько последовательных тестов

Опция `--count` *(или `-c`)* задаёт количество замеров:
```bash
yaspeedtest --count 5
```
CLI выполнит 5 последовательных измерений и выведет агрегированный результат, рассчитанный внутри клиента.

### ▶ Вывод в формате JSON

Для автоматизации или логирования доступен JSON-вывод:
```bash
yaspeedtest --json
```
Пример:
